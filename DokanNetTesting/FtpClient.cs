...
namespace DokanNetTesting
{
    public interface IFtpClient
    {
        ...
        [NotNull]
        Task DownloadDirectoryAsync([NotNull] string remotePath, [NotNull] string localPath);
    ...
    public class FtpClient : IFtpClient
    {
        ...
        public async Task DownloadDirectoryAsync(string remotePath, string localPath) {
            IReadOnlyCollection<FtpFileSystemInfo> list = await ListDirectoryTreeAsync(remotePath);

            int prefix = localPath.Length;

            // sync directories to match server structure
            FtpDirectoryInfo[] ftpDirs = list.OfType<FtpDirectoryInfo>().ToArray();
            DirectoryInfo[] localDirs = new DirectoryInfo(localPath).GetDirectories("*", SearchOption.AllDirectories);
            IEnumerable<Tuple<FtpDirectoryInfo, DirectoryInfo>> joinDirs =
                ftpDirs.FullOuterJoin(localDirs,
                            ...
                            o => {
                                Debug.Assert(o != null, nameof(o) + " != null");
                                return o.FullName.Substring(prefix).Replace('\\', '/');
                            },
            ...
            foreach (Tuple<FtpDirectoryInfo, DirectoryInfo> pair in joinDirs) {
                ...
                if (pair.Item1 == null) {
                    ...
                    string localDirPath = localPath + pair.Item1.FullName.Replace('/', '\\');
                    if (pair.Item2 == null) {
                        Directory.CreateDirectory(localDirPath);
                    }

                    Directory.SetLastWriteTime(localDirPath, pair.Item1.Modify);
                }
            }

            // sync files to match server, override local
            FtpFileInfo[] ftpFiles = list.OfType<FtpFileInfo>().ToArray();
            FileInfo[] localFiles = new DirectoryInfo(localPath).GetFiles("*", SearchOption.AllDirectories);
            ...
            // parallel foreach, max threads: 10
            ...
            IEnumerable<Task> tasks = joinFiles.Select(async pair => {
                ...
                try {
                    if (pair.Item1 == null) {
                        ...
                        if (download) {
                            string localFilePath = localPath + pair.Item1.FullName.Replace('/', '\\');
                            Console.WriteLine(@"DOWNLOAD: " + localFilePath);
                            await DownloadFileAsync(pair.Item1.FullName, localFilePath);
                            File.SetLastWriteTime(localFilePath, pair.Item1.Modify);
                        }
                    }
                } finally {
                    // ReSharper disable once AccessToDisposedClosure
